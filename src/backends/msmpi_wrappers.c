...
int (*msmpi_ialltoallw)(const void *, const int *, const int *,
    const int *, void *, const int *, const int *, const int *, MPI_Comm,
    MPI_Request *);
int (*msmpi_type_create_struct)(int, const int *, const MPI_Aint *,
    const int *, MPI_Datatype *);
int (*msmpi_type_struct)(int, const int *, const MPI_Aint *,
    const int *, MPI_Datatype *);
int (*msmpi_type_get_contents)(MPI_Datatype, int, int, int,
...
int msmpi_wrap_type_create_struct(int count,
    ...
    int ret = msmpi_type_create_struct(count, array_of_blocklengths,
        array_of_displacements, (const int *)array_of_types, newtype);
    dtypes_restore_inplace((MPI_Datatype *)array_of_types, count);
    return ret;
}

int msmpi_wrap_type_struct(int count,
    const int *array_of_blocklengths, const MPI_Aint *array_of_displacements,
    const MPI_Datatype *array_of_types, MPI_Datatype *newtype) {
    dtypes_compress_inplace(array_of_types, count);
    int ret = msmpi_type_struct(count, array_of_blocklengths,
        array_of_displacements, (const int *)array_of_types, newtype);
    dtypes_restore_inplace((MPI_Datatype *)array_of_types, count);
    return ret;
}
...
int msmpi_wrap_type_get_contents(MPI_Datatype datatype, int max_integers,
    ...
    int ret = msmpi_type_get_contents(datatype, max_integers, max_addresses,
...
int msmpi_wrap_comm_spawn_multiple(int count, char *array_of_commands[],
     ...
     * 4-byte int on MSMPI); compress before the call, restore after. */
    dtypes_compress_inplace(array_of_info, count);
    int ret = msmpi_comm_spawn_multiple(count, array_of_commands,
