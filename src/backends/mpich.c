...
static void init_mpich_error_codes(void) {
    /* MPICH public mpi.h error-class values (not Open MPI numbering). */
    MPI_SUCCESS = 0;
    MPI_ERR_BUFFER = 1;
    MPI_ERR_COUNT = 2;
    MPI_ERR_TYPE = 3;
    ...
    MPI_ERR_DISP = 52;
    MPI_ERR_ASSERT = 53;
    /* MPICH publishes process failure as MPIX_ERR_PROC_FAILED (101).
     * Native class 55 is MPI_ERR_RMA_RANGE, not a process-failure class.
     * MPI_ERR_PROC_FAIL_STOP has no corresponding native class; use a
     * negative unsupported sentinel that cannot collide with nonnegative
     * native classes (do not use 102: that is PROC_FAILED_PENDING). */
