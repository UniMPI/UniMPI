 ...
 * handles through the production binder (no test-only setters).
 *
 ...
 *   Ibarrier / Rput / File_iread producers
 *   Mprobe -> Imrecv -> Wait
 *   Mprobe -> Mrecv
...
static void test_representative_producers(void) {
    ...
    request = 0;
    assert(unimpi.rput(
               NULL, 1, (MPI_Datatype)1, 0, (MPI_Aint)0, 1, (MPI_Datatype)1,
               (MPI_Win)1, &request) == 0);
    assert(request == facade_request(FAKE_NATIVE_REQUEST));

    request = 0;
    assert(unimpi.file_iread(
               (MPI_File)1, NULL, 1, (MPI_Datatype)1, &request) == 0);
    assert(request == facade_request(FAKE_NATIVE_REQUEST));
